...
const api = {
  ...
  downloadAndParseMrpack: (options: any) => electronAPI.ipcRenderer.invoke('download-parse-mrpack', options),
  installMrpack: (options: any) => electronAPI.ipcRenderer.invoke('install-mrpack', options),
  onInstallationProgress: (callback: any) => electronAPI.ipcRenderer.on('installation-progress', (_event, value) => callback(value)),
